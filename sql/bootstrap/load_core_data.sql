...
-- scripts/publish/publish_core_dataset.py.
--
-- Loads the processed CSVs into bracketballer_dev. Run from the project root:
--   PGPASSWORD=test psql -h localhost -U postgres -d bracketballer_dev -v ON_ERROR_STOP=1 -f sql/bootstrap/load_core_data.sql
--
...
\copy schools (id, name) FROM 'data/processed/core/schools.csv' WITH (FORMAT csv, HEADER true)
...
\copy players_staging FROM 'data/processed/core/processed_players.csv' WITH (FORMAT csv, HEADER true)
...
\copy player_position_maps (player_id, position) FROM 'data/processed/core/player_positions.csv' WITH (FORMAT csv, HEADER true)
