...
Run from the project root, after the V10 migration has created the table:
    python process_torvik_defensive_data.py
    python process_torvik_defensive_data.py --refresh        # re-download all
    python process_torvik_defensive_data.py --first 2020 --last 2025
"""
...
def load_env_file(path=".env"):
    ...
    The project keeps DB credentials in .env but doesn't export them, so read
    them here. Existing process env vars win, so DATABASE_URL etc. still override.
    """
    env_path = Path(path)
...
def connection_dsn():
    """Build a libpq DSN from the environment (.env vars), or use DATABASE_URL."""
    ...
    if not (user and dbname):
        raise SystemExit(
            ...
            "(see .env). e.g. export $(grep -v '^#' .env | xargs)"
        )
